...
function XuatXuPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await xuatXuApi.getAll();
      setData(res || []);
    } catch (error) {
      handleError(error);
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingId) {
        await xuatXuApi.update(editingId, { ten: ten.trim() });
      } else {
        await xuatXuApi.create({ ten: ten.trim() });
      }
