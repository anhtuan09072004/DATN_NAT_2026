...
function ThuongHieuPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await thuongHieuApi.getAll();
      setData(res || []);
    } catch (error) {
      handleError(error);
    }
  ...
  const handleSubmit = async () => {
    ...
    if (!ten.trim()) {
      setErrorMsg("Vui lòng nhập tên thuong hieu!");
      return;
    }
    ...
    try {
      ...
      if (editingId) {
        await thuongHieuApi.update(editingId, { ten: ten.trim() });
      } else {
        await thuongHieuApi.create({ ten: ten.trim() });
      }
